...
let db = JSON.parse(localStorage.getItem('salesDB_v7')) || defaultData;
let isAdmin = false;
...
function switchView() {
    ...
    data.rows.forEach(row => {
        ...
        Object.values(row).forEach(val => {
            ...
            if(String(val).includes('%')) td.className = 'highlight-cell';
            tr.appendChild(td);
        });
...
function saveJsonData() {
    try {
        ...
        localStorage.setItem('salesDB_v7', JSON.stringify(db));
        closeAdminPanel();
        switchView();
        alert("Data updated locally! Remember to share the JSON text with others so they can update their browsers too.");
    } catch (e) {
